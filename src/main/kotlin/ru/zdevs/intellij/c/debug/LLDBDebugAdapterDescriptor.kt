...
class LLDBDebugAdapterDescriptor(
    ...
    override fun startServer(): ProcessHandler {
        var command =  if (options is DAPRunConfigurationOptions) {
            ...
            null
        if (command.isNullOrEmpty() || DEBUGGER_EXEC_NAME == command) {
            command = Utils.findExecutableInPATH(DEBUGGER_EXEC_NAME)
        }
        if (command.isNullOrEmpty()) {
            throw ExecutionException("LLDB not found. Make sure it is installed properly (and `$DEBUGGER_EXEC_NAME` available in PATH), and restart the IDE.")
        }
    ...
    companion object {
        val DEBUGGER_EXEC_NAME = if (SystemInfo.isWindows) "lldb-dap.exe" else "lldb-dap"
    }
}